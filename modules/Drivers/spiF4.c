...
spiState_t spiTxRxDma(spi_t* spi, uint8_t* src, uint8_t* dst, uint8_t len, uint16_t timeout_ms) {
    if (NULL == spi) {
        return SPIState_INVALID_PARAM;
    }
    if (xSemaphoreTake(spi->semBusy, timeout_ms / portTICK_PERIOD_MS) != pdTRUE) {
    ...
    spiState_t state = spi->state;
    xSemaphoreGive(spi->semBusy);
    return state;
...
spiState_t spiTxRxByte(spi_t* spi, uint8_t sendByte, uint8_t* receiveByte, uint16_t timeout_ms) {
    ...
    spi->pSpi->DR;
    spi->pSpi->SR = 0;
    spi->pSpi->DR = sendByte;

    while (spi->pSpi->SR && !SPI_SR_RXNE) {
        if (xTaskCheckForTimeOut(&xTimeOut, &xTicksToWait) != pdFALSE){
            xSemaphoreGive(spi->semBusy);
            return SPIState_TIMEOUT;
        }
